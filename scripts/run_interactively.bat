...
ssh -tt -J %user%@labta.math.unipd.it %user%@labsrv8.math.unipd.it "srun --job-name=gligen_boxdiff --chdir=/home/dvezzaro/hf_boxdiff --partition=allgroups --time=0-01:00:00 --mem=12G --gres=gpu:1 --cpus-per-task=4 --pty bash -c 'source /conf/shared-software/anaconda/etc/profile.d/conda.sh && conda activate hf_boxdiff && python -W ignore /home/dvezzaro/hf_boxdiff/run_gligen_boxdiff.py && conda deactivate'"